...
function App() {
  ...
  const [filters, setFilters] = useState<Filters>({ rentMin: '', rentMax: '', rooms: '', propertyType: '' })
  const [listings, setListings] = useState<ListingCollection>({ type: 'FeatureCollection', features: [], meta: { count: 0 } })
        ...
        onEachFeature={(feature, layer) => {
          ...
          layer.bindTooltip(
            `<strong>${props.name}</strong><br/>Listings: ${props.listing_count}<br/>Median: ${
              props.median_eur_per_m2 == null ? 'No data' : `€${props.median_eur_per_m2.toFixed(2)}/m²`
            }`,
          )
        }}
      />
  ...
  return (
    <div className="app-shell">
      <header>
        ...
        <div className="status">{loading ? 'Updating map…' : `${listings.meta.count} visible listings`}</div>
      </header>

      <aside className="filters panel">
        <h2>Filters</h2>
        <label>Minimum rent (€)<input type="number" min="0" value={filters.rentMin} onChange={(e) => setFilters({ ...filters, rentMin: e.target.value })} /></label>
        <label>Maximum rent (€)<input type="number" min="0" value={filters.rentMax} onChange={(e) => setFilters({ ...filters, rentMax: e.target.value })} /></label>
        <label>Rooms<select value={filters.rooms} onChange={(e) => setFilters({ ...filters, rooms: e.target.value })}><option value="">Any</option>{[1,2,3,4,5].map((value) => <option key={value}>{value}</option>)}</select></label>
        <label>Property type<select value={filters.propertyType} onChange={(e) => setFilters({ ...filters, propertyType: e.target.value })}><option value="">Any</option><option>apartment</option><option>studio</option><option>townhouse</option></select></label>
        <button onClick={() => setFilters({ rentMin: '', rentMax: '', rooms: '', propertyType: '' })}>Clear filters</button>
      </aside>
      ...
      <main>
        ...
        <MapContainer bounds={initialBounds} className="map" zoomControl>
          ...
          {listings.features.map((feature) => {
            ...
            return (
              <CircleMarker key={p.id} center={[lat, lng]} radius={6} pathOptions={{ fillOpacity: 0.85, weight: 1 }}>
                <Popup>
                  <div className="popup">
                    ...
                    <span>Price: {p.eur_per_m2 ? `€${p.eur_per_m2.toFixed(2)}/m²` : 'Unknown'}</span>
        ...
        <section className="summary panel">
          <h2>Current view</h2>
          <div><strong>{listings.meta.count}</strong><span>Listings</span></div>
          <div><strong>{listings.meta.median_eur_per_m2 == null ? '—' : `€${listings.meta.median_eur_per_m2.toFixed(2)}`}</strong><span>Median €/m²</span></div>
        </section>

        <section className="legend panel">
          <h2>Median €/m²</h2>
          {[['< 16', 14], ['16–20', 18], ['20–24', 22], ['24–28', 26], ['28+', 30]].map(([label, value]) => (
            <div className="legend-row" key={String(label)}><span style={{ background: colorFor(Number(value)) }} />{label}</div>
          ))}
        </section>
      </main>
    </div>
